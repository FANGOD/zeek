...
TeredoAnalyzer::TeredoAnalyzer()
	: zeek::packet_analysis::Analyzer("TEREDO")
	{
	}
...
bool TeredoAnalyzer::AnalyzePacket(size_t len, const uint8_t* data, Packet* packet)
	...
	// TODO: we don't get an orig flag in via the caller in packet analyzers
	// if ( orig )
	// 	valid_orig = false;
	// else
	// 	valid_resp = false;
